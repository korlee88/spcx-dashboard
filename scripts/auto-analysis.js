 ...
 * GitHub Actions에서 1일 1회 실행 (매일 KST 10:00)
...
async function collectGoogleTrends(dateStr) {
  try {
    const data = await geminiPost({
      ...
      contents: [{
        role: 'user',
        parts: [{ text: `Search Google Trends data for the keyword "${cfg.company_en}" in the United States for the past 7 days (around ${dateStr}).
...
async function main() {
  ...
  // 핵심 인물 X 센티먼트를 buyIndex에 반영 (독립 신호)
  let muskXAdj = 0;

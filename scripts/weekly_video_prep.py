...
def summarize(sessions):
    ...
    # 이번 영상이 다루는 '보도 주간'의 시작일(윈도우 내 가장 오래된 세션 날짜).
    # 이 날짜 이전(주간 범위 밖)에 발행된 뉴스는 점수가 아무리 높아도 BEST 픽에서
    # 밀어낸다 — 지난 IPO 기사(역대 최고점수)가 몇 주 뒤 영상에서도 계속 1위를
    # 차지해 씬1 이미지·헤드라인이 매주 같은 사건을 반복하는 문제를 차단. 단, 주간 내
    # 뉴스가 하나도 없으면 모두 동률(0)이 되어 기존 (recent, score) 순으로 자연 폴백.
    _week_start = (sessions[-1].get("date", "") or "")[:10]
    def _in_window(n):
        d = (n.get("date", "") or "")[:10]
        return 1 if (_week_start and d and d >= _week_start) else 0

    def _bull_sort_key(n):
        is_recent = 1 if _days_ago(n.get("date", "")) <= RECENT_NEWS_DAYS else 0
        return (_in_window(n), is_recent, n.get("score", 0))

    def _bear_sort_key(n):
        is_recent = 1 if _days_ago(n.get("date", "")) <= RECENT_NEWS_DAYS else 0
        return (_in_window(n), is_recent, -n.get("score", 0))   # 악재는 점수가 낮을수록(더 부정적) 우선
...
★★ 가장 중요 — 매 영상마다 이미지가 반드시 달라야 한다. 일반적인 도시 풍경만 반복하지 말고,
   아래 ①②③ '이번 주 실제 내용'을 각 씬 이미지의 중심 소재로 삼아 매번 새로 작성한다:
  ① 선순위 선택 뉴스(이번 주 실제 핵심 뉴스를 '장면'으로 시각화):
     · 씬0 = 이번 주 주가 변동 원인/대표 분위기 → "{movement_reason_str}" (상승주는 활기찬, 하락주는 긴장된 톤)
     · 씬1 = 이번 주 BEST 호재의 구체적 장면 → "{top_bull_title}" 를 영어 비주얼 개념으로 옮겨 중심에 배치
  ② 후순위 스케줄(다가오는 일정을 미래 지향적 장면으로):
     · 씬2 = 다음주 예정 이벤트 → "{next_events_str}" 를 내다보는 장면
  ③ 미래 기술(모든 씬 배경에 깔기): {future_tech}
※ 한글 뉴스·일정은 그 '시각적 개념'만 영어로 옮겨 묘사한다(글자·로고·고유명사 텍스트는 그리지 않는다 — 장면으로만).
...
※ 아래 IMAGE_PROMPT는 색감·구도 '뼈대'일 뿐이다 — {scene0_subject}/{scene1_subject}/{scene2_subject} 같은 일반 주제어나 도시 묘사를 그대로 베끼지 말고, 위 ①②③의 이번 주 구체 소재로 중심 비주얼을 교체해 매번 새로 쓴다.

IMAGE_PROMPT_0: [씬0 — 16:9 landscape · 이번 주 변동 원인/핵심 뉴스를 상징하는 {scene0_subject} 장면을 중심으로, {future_tech} 배경, 서울 한강 야경·남산타워·63빌딩·롯데타워 도심 스카이라인, K-tech 보라빛 미래적 분석 분위기, glowing city lights bokeh, ultra-high resolution, 16:9 landscape, no text, no letters, no watermark, no logo]
IMAGE_PROMPT_1: [씬1 — 16:9 landscape · 이번 주 BEST 호재의 구체적 장면을 중심에 둔 {scene1_subject} 역동적 비주얼, {future_tech} 배경, 한강 다리 초록빛 성장·상승 에너지, 서울 도심 sunlit modern skyline vibrant, ultra-high resolution, 16:9 landscape, no text, no letters, no watermark, no logo]
IMAGE_PROMPT_2: [씬2 — 9:16 vertical · 다음주 예정 일정을 내다보는 미래 비전 장면을 중심으로, {scene2_subject}·{future_tech} 결합, 한강 야경·서울 스카이라인·광화문 광장 배경, 마젠타·골드빛 영감적 미래 무드, 황금빛 태양·별빛 반짝임, ultra-high resolution, 9:16 vertical, no text, no letters, no watermark, no logo]"""
...
def _build_prompt(summary):
    ...
    # 이미지 프롬프트가 '이번 주 실제 호재'를 그리도록, 씬1이 다루는 BEST 호재 1건의 제목을 따로 뽑는다.
    _tb = summary.get("top_bullish") or []
    top_bull_title = _tb[0]["title"] if _tb else "이번 주 주요 호재 (구체 뉴스 없음 — 일반 성장 비전)"
...
3. 미래 비전 전달력: 씬2(다음주 전망)와 이미지 프롬프트가 {company_ko}의 미래 기술·사업 비전을
   ...
   {future_tech}
3-1. 이미지 프롬프트 차별성: IMAGE_PROMPT_0/1/2가 이번 주 실제 뉴스·일정을 반영한 '서로 다른' 장면인가?
   (씬0=변동 원인, 씬1=BEST 호재의 구체 장면, 씬2=다음주 일정 전망) — 세 프롬프트가 일반 도시 풍경만
   반복하거나 서로 거의 동일하면, 1차 초안의 씬 본문에 담긴 구체 소재를 살려 각 씬을 뚜렷이 구분되게
   보강한다 (새 사실 창작 금지, 글자/로고 묘사 금지 규칙 유지).
...
def build_scene_image(scene, summary, font_reg, font_bold, bg_path: Path | None = None):
    ...
    sf_ct        = fnt(soft_reg,  46)   # 호재 내용 본문 — 요청대로 한 단계 작게(50→46)
